    ...
    <form id="billing-form" method="POST" action="{{ route('bill.store') }}">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-4">
                ...
                <input type="date" class="form-control" id="billDate" name="billDate" value="{{ date('Y-m-d') }}">
            </div>
        </div>
        <div class="form-group col-4">
        ...
        <button type="button" class="btn btn-primary mb-5" onclick="addProductRow()">Add Product Row</button>
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th>Discount Type</th>
        ...
        <button type="button" class="btn btn-primary" onclick="calculateFinalAmount()">Calculate Final Amount</button>
...
<script>
    ...
    function getCustomers(customerType) {
        $.ajax({
            ...
            success: function (data) {
                $('#customerName').empty();
                $('#customerName').append('<option value="">Select Customer</option>'); // Add default option

                // Populate customer dropdown based on customerType
                $.each(data, function (index, customer) {
                    $('#customerName').append('<option value="' + customer.id + '">' + customer.name + '</option>');
                });
            },
    ...
    <td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">Delete</button></td>
            ...
            `;
                ...
                calculateTotalAmount(); // Recalculate total amount after adding a product row
            }
        });
    }
    ...
    function addRow2() {
        ...
        <tr>
            <td>
                <select class="form-control discountType" name="discountType[]" onchange="calculateFinalAmount()">
                    <option value="Flat">Flat</option>
                    <option value="Percentage">Percentage</option>
                </select>
            </td>
            <td><input type="number" class="form-control discount" name="discount[]"  onchange="calculateFinalAmount()"></td>
            <td><input type="number" class="form-control vat" name="vat[]" onchange="calculateFinalAmount()"></td>
    ...
    function calculateTotal(element) {
        ...
        calculateTotalAmount(); // Recalculate total amount after updating row total
    }
    ...
    function calculateTotalAmount() {
        ...
        // Do not update final amount here, only when explicitly calculating final amount
    }
    ...
    function calculateFinalAmount() {
        ...
        let finalAmount = totalAmount; // Initialize final amount with total amount
        document.querySelectorAll('#bill-items2 tr').forEach(row => {
            const discountType = row.querySelector('select[name="discountType[]"]').value;
            const discountAmount = parseFloat(row.querySelector('input[name="discount[]"]').value) || 0;
    ...
    function removeRow(button) {
        ...
        calculateFinalAmount(); // Recalculate final amount after removing a row
    }

    // Calculate initial total amount on page load
    document.addEventListener('DOMContentLoaded', function () {
        calculateTotalAmount(); // Ensure initial total amount is calculated
    });
    ...
    // Update the final amount field in the discount/VAT table
    document.querySelectorAll('#bill-items2 tr').forEach(row => {
        row.querySelector('input[name="finalAmount"]').value = finalAmount.toFixed(2);
    });
}
</script>
@endsection
